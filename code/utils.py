...
def dense2sparse(matrix: np.ndarray):
    mat_coo = coo_matrix(matrix)    # 将一个普通的密集矩阵或者其他稀疏矩阵类型转换为 COO 格式的稀疏矩阵，由三个一维数组 (datat, (row, col)) 表示
...
def calculate_evaluation_metrics(pred_mat, pos_edges, neg_edges):
    ...
    pred_labels = np.hstack((pos_pred_socres, neg_pred_socres))  # 一维数组
...
def load_data():
    cir_sim = pd.read_csv("../mydata/gene_seq_sim.csv", index_col=0, dtype=np.float32).to_numpy()
    drug_sim=pd.read_csv("../mydata/drug_str_sim.csv", index_col=0, dtype=np.float32).to_numpy()
    drug_cir_ass =pd.read_csv("../mydata/association.csv", index_col=0).to_numpy().T
...
def get_all_samples(conjunction):   # 做平衡训练的
    pos = []
    neg = []
...
class InnerProductDecoder(nn.Module):
    ...
    def __init__(self, input_dim, name, num_d, act=torch.sigmoid):
        ...
        # self.att_drug = nn.Parameter(t.rand(2), requires_grad=True)
        # self.att_cir = nn.Parameter(t.rand(2), requires_grad=True)
...
class InnerProductDecoder1(nn.Module):
    """Decoder model layer for link prediction."""
    def __init__(self, num, feature_size):
        super(InnerProductDecoder1, self).__init__()
        self.num_d = num
        self.W = Parameter(torch.randn(feature_size, feature_size),requires_grad=True)
